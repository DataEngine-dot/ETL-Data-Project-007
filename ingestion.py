...
def extract_table(conn: Connection, table: str, since: str = None):
    ...
    try:
        ...
        # Only run incremental query if since is actually set and non-empty
        if since is not None and since != "":
            logger.info(f"Extracting {table} since={since!r}")  # For debug
            query = f'{base_query} WHERE last_updated > $1'
            rows = conn.run(query, (since,)) # In Python, always pass parameters as a tuple—even if there’s only one.
        else:
            logger.info(f"Extracting {table} (full load, no since)")  # For debug
    ...
    try:
        ...
        for table in TABLES:
            try:
                ...
                if result_rows:
                    max_ts = max([r["last_updated"] for r in result_rows])
        ...
        # This is important for Step Function handoff!
        return {
            "status": "success",
            "rows": total_rows,
            "s3_keys": all_s3_keys
