# Create a CloudWatch Event Rule that triggers every 30 minutes
resource "aws_cloudwatch_event_rule" "lambda_schedule" {
  name                = "every-30-minutes"
  schedule_expression = "rate(30 minutes)"

# 1) To remove the Event Rule from Terraform, you must manually delete its Target in AWS Console:
  #    AWS Console → EventBridge → Rules → every-30-minutes → Remove all associated Targets
...
# Attaches the cloudwatch event rule to Lambda function
...
# Gives permission for EventBridge to invoke the Lambda function
